...
import { Search, Shield, Clock, Star, ChevronDown, ChevronUp } from 'lucide-react';
...
const FAQS = [
  { q: 'How do I book a car?', a: 'Browse available cars, choose your pickup and return dates, and confirm the booking. The owner approves within 24 hours.' },
  { q: 'Can I list my own car?', a: 'Yes — register as a Car Owner, list your car with photos and pricing, and our team approves the listing within one business day.' },
  { q: 'What if I need to cancel?', a: 'Customers can cancel any pending or approved booking from their dashboard before the pickup date.' },
  { q: 'Is payment handled in the app?', a: 'Payment is currently arranged directly between customers and owners after booking approval.' },
];
  ...
  return (
    <div className="border-b border-gray-100 dark:border-gray-800 py-5">
      <button onClick={() => setOpen((o) => !o)} className="flex w-full items-center justify-between text-left">
        <span className="font-medium text-gray-900 dark:text-luxury-ivory">{q}</span>
        {open ? <ChevronUp className="h-5 w-5 text-gray-400 shrink-0" /> : <ChevronDown className="h-5 w-5 text-gray-400 shrink-0" />}
      </button>
      {open && <p className="mt-3 text-sm text-gray-500 dark:text-gray-400 leading-relaxed">{a}</p>}
    </div>
  );
};
...
const Home = () => {
  ...
  const handleSearch = (e) => {
    e.preventDefault();
    navigate(`${ROUTES.CARS}?q=${encodeURIComponent(search)}`);
  };
  ...
  return (
    <div>
      ...
      <section className="relative overflow-hidden bg-luxury-charcoal">
        ...
        <div className="relative mx-auto max-w-7xl px-6 py-28 sm:py-40 lg:px-8">
          <motion.div
            ...
            className="max-w-2xl"
          >
            <p className="mb-3 text-sm font-medium uppercase tracking-widest text-luxury-gold">Premium Car Rental Marketplace</p>
            ...
            <p className="mt-5 text-lg text-luxury-ivory/70 leading-relaxed">
              Rent from local owners or list your own car. Book premium vehicles near you — in minutes.
            </p>

            <form onSubmit={handleSearch} className="mt-10 flex max-w-xl items-center gap-2 rounded-full bg-white/10 p-1.5 backdrop-blur-sm border border-white/20">
            ...
            <div className="mt-8 flex flex-wrap gap-4 text-sm text-luxury-ivory/60">
              {['No hidden fees', 'Flexible cancellation', 'Verified owners'].map((t) => (
                <span key={t} className="flex items-center gap-1.5">
                  <span className="h-1.5 w-1.5 rounded-full bg-luxury-gold" /> {t}
                </span>
              ))}
            </div>
          </motion.div>
      ...
      <section className="bg-white dark:bg-luxury-charcoal py-20 px-6">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="grid grid-cols-1 gap-8 sm:grid-cols-3">
            {[
              { icon: Search, title: 'Browse & Search', desc: 'Filter by location, date, transmission, fuel type and price to find the right car.' },
              { icon: Shield, title: 'Book Securely', desc: 'Select your dates, confirm the pickup location, and send the booking request.' },
              { icon: Clock, title: 'Drive Same Day', desc: 'Owner approves the booking and you pick up the keys. That simple.' },
              ...
              >
                ...
                <h3 className="mb-2 font-display text-lg font-semibold text-gray-900 dark:text-luxury-ivory">{title}</h3>
      ...
      <section className="bg-luxury-ivory dark:bg-gray-950 py-20 px-6">
        <div className="mx-auto max-w-7xl">
          <div className="mb-10 flex items-end justify-between">
            <div>
              <h2 className="font-display text-3xl font-bold text-gray-900 dark:text-luxury-ivory">Recently listed</h2>
              <p className="mt-1 text-gray-500">Fresh cars from our growing community</p>
            </div>
            <Link to={ROUTES.CARS} className="text-sm font-medium text-primary-600 hover:underline">
              Browse all →
            </Link>
          </div>
          {status === 'loading' ? (
      ...
      <section className="bg-white dark:bg-luxury-charcoal py-20 px-6">
        <div className="mx-auto max-w-7xl">
          <div className="mb-12 text-center">
            <h2 className="font-display text-3xl font-bold text-gray-900 dark:text-luxury-ivory">What our community says</h2>
          </div>
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-3">
            {TESTIMONIALS.map(({ name, role, text, rating }) => (
              ...
              >
                ...
                <div className="mt-4">
                  <p className="font-semibold text-gray-900 dark:text-luxury-ivory">{name}</p>
      ...
      <section className="bg-luxury-ivory dark:bg-gray-950 py-20 px-6">
        <div className="mx-auto max-w-3xl">
          <h2 className="mb-10 text-center font-display text-3xl font-bold text-gray-900 dark:text-luxury-ivory">Frequently asked questions</h2>
      ...
      <section className="bg-primary-700 px-6 py-16 text-center">
        <h2 className="font-display text-3xl font-bold text-white">Ready to list your car?</h2>
        <p className="mt-3 text-primary-200">Earn money from your car when you're not using it.</p>
        <Link
          to={ROUTES.REGISTER}
